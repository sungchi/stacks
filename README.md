...
현재 기본 게임은 KST 시간 seed마다 같은 40장 동식물 덱을 제공하는 `매시간 네 정원 수확`이다. 네 장을 한 정원에 모아 합으로 수확하고, 수확한 네 장에서 시계방향 정원 꼭대기까지 이어지는 하나의 연쇄 `×1~×7` 또는 같은 생물군 네 장 `×3` 중 더 높은 배수를 곱해 별 목표에 도전한다. 기존 무한 정원과 캠페인은 명시 모드로 보존한다.

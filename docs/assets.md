...
- 매 seed는 후보에서 식물 5종과 동물 5종을 결정적으로 선택한다.
- 같은 seed의 재시도와 저장 복원은 같은 `speciesId`, `variantId`, 카드명, 이미지 경로를 유지한다.
- 선택된 각 종은 서로 다른 숫자 네 장에 배정되고, 숫자 0~9도 각각 네 장을 유지한다.
- 종은 같은 종 수확 `×5`에 사용하는 규칙 데이터이며, 이미지 파일 자체에는 별도 점수 속성이 없다.
- 참조 파일이 누락되면 `card_locked_unknown.png`로 대체하되 `speciesId`와 판정은 유지한다.
